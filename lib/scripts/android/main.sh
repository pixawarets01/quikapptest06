...
if [ ! -f gradlew ]; then
    ...
    if [[ "${WORKFLOW_ID:-}" == "android-publish" ]] || [[ "${WORKFLOW_ID:-}" == "combined" ]]; then
        ...
        if flutter build apk --release && \
           flutter build appbundle --release; then
    ...
    log "🛑 Stopping existing Gradle daemons..."
    ./gradlew --stop
    ...
    if [[ "${WORKFLOW_ID:-}" == "android-publish" ]] || [[ "${WORKFLOW_ID:-}" == "combined" ]]; then
        # Build both APK and AAB for publish workflows
        log "📦 Building APK and AAB..."
        if ./gradlew assembleRelease bundleRelease \
            --no-daemon \
            --parallel \
            --max-workers=4 \
            -Dorg.gradle.jvmargs="-Xmx4G -XX:MaxPermSize=512m -XX:+UseParallelGC" \
