...
# Create gradle.properties with optimized settings if it doesn't exist
...
if [ ! -f gradlew ]; then
    ...
    # Build with specific Gradle settings
    log "🔨 Building with optimized Gradle settings..."
    if [[ "${WORKFLOW_ID:-}" == "android-publish" ]] || [[ "${WORKFLOW_ID:-}" == "combined" ]]; then
        # Build both APK and AAB for publish workflows
        log "📦 Building APK and AAB..."
        if flutter build apk --release \
            --dart-define=GRADLE_OPTS="-Dorg.gradle.jvmargs=-Xmx2048m -XX:MaxMetaspaceSize=512m" && \
           flutter build appbundle --release \
            --dart-define=GRADLE_OPTS="-Dorg.gradle.jvmargs=-Xmx2048m -XX:MaxMetaspaceSize=512m"; then
    ...
    # Pre-warm Gradle daemon with optimized settings
    ...
    if [[ "${WORKFLOW_ID:-}" == "android-publish" ]] || [[ "${WORKFLOW_ID:-}" == "combined" ]]; then
        ...
        if GRADLE_OPTS="-Dorg.gradle.jvmargs=-Xmx2048m -XX:MaxMetaspaceSize=512m" \
           ...
           --parallel --max-workers=4 --daemon \
           -Dorg.gradle.configureondemand=true \
           -Dkotlin.compiler.execution.strategy=in-process \
           -Dkotlin.incremental=false; then
            log "✅ APK and AAB build completed successfully"
        else
            log "❌ APK and AAB build failed"
            exit 1
